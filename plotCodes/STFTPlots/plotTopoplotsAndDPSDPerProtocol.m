function plotTopoplotsAndDPSDPerProtocol(dataLog,freqBandToPlot,refChan)

if ~exist('freqBandToPlot','var')||isempty(freqBandToPlot); freqBandToPlot = 'AllGamma'; end;
...
[~,~,plotPos] = getPlotHandles(length(rowNum),length(colNum),gridPos,0.002,0.002);
                                        ...
                                        for ao=1:aoLen
                                            for av=1:avLen
                                                for at=1:atLen     
                                                    ...
                                                    clear hPlots
                                                    hPlots = getPlotHandles(2,2,plotPos{iX,iY},0.002,0.002);
                                                    ...
                                                    for freqBandToPlot = {'LGamma','HGamma'}
                                                        clear peakPower
                                                        switch cell2mat(freqBandToPlot)
                                                            case 'LGamma'
                                                        ...
                                                        subplot(hPlots(topoRow,topoCol)); hold on;
                                                        topoplot(peakPower,chanlocs,'electrodes','off','style','both','drawaxis','off','nosedir',noseDir); caxis([-3 3]);
                                                        hold off; 
                                                        drawnow
                                                    end
